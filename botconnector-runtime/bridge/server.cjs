...
function starterMaps(models){
  const free=models.filter(m=>m&&m.starterFreeEligible===true);
  const aliasToCanonical=new Map(),canonicalToAlias=new Map();
  for(const m of free){
    let alias=starterAlias(m.id),base=alias,n=2;
    while(aliasToCanonical.has(alias)&&aliasToCanonical.get(alias)!==m.id)alias=base+'-'+n++;
    aliasToCanonical.set(alias,m.id);canonicalToAlias.set(m.id,alias);
  }
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(url.pathname==='/.well-known/openid-configuration')return json(res,200,{issuer:ISSUER,authorization_endpoint:ISSUER+'/authorize',token_endpoint:ISSUER+'/token',userinfo_endpoint:ISSUER+'/userinfo',jwks_uri:ISSUER+'/jwks',response_types_supported:['code'],subject_types_supported:['public'],id_token_signing_alg_values_supported:['RS256'],scopes_supported:['openid','profile','email'],token_endpoint_auth_methods_supported:['client_secret_basic','client_secret_post'],claims_supported:['sub','iss','aud','exp','iat','nonce','email','email_verified','name','preferred_username']});
